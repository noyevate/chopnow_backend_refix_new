...
async function placeOrder(req, res) {
    try {
        ...
        const result = await sequelize.transaction(async (t) => {
            ...
            // --- ✅ Restaurant open check (inside transaction so it's atomic) ---
            const restaurant = await Restaurant.findByPk(orderData.restaurantId, { transaction: t });
            ...
            const openStatus = isRestaurantOpen(restaurant, orderData.orderType);

            if (!openStatus.isOpen) {
                const err = new Error(openStatus.reason);
                err.statusCode = 409;
                err.requiresRefund = true; // flag for your payment handler
                throw err;
            }
            // --- End of check ---
        ...
        logger.info(`Order placed successfully`, { controller: 'orderController', orderId: order.id, endpoint: `placeOrder` });
        ...
        // ✅ Handle the open-status rejection cleanly
        if (error.statusCode === 409) {
            logger.warn(`Order blocked - restaurant closed: ${error.message}`, { controller: 'orderController' });

            // TODO: if error.requiresRefund === true, trigger your payment gateway refund here

            return res.status(409).json({
                status: false,
                message: error.message,
                requiresRefund: error.requiresRefund || false,
            });
        }
        ...
        logger.error(`Failed to place order: ${error.message}`, { controller: 'orderController', endpoint: `placeOrder` });
